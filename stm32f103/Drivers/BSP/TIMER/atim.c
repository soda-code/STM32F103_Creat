...
/*高级定时器PWM*/
//static TIM_HandleTypeDef g_atimx_pwm_chy_handle;     /* 定时器x句柄 */
//static TIM_OC_InitTypeDef g_atimx_oc_pwm_chy_handle; /* 定时器输出句柄 */
...
/*高级定时器输出比较模式*/
static TIM_HandleTypeDef g_atimx_comp_pwm_handle;                              /* 定时器x句柄 */

/*互补输出带死区控制*/
static TIM_HandleTypeDef g_atimx_cplm_pwm_handle;                              /* 定时器x句柄 */
...
void HAL_TIM_OC_MspInit(TIM_HandleTypeDef *htim)
{
    if (htim->Instance == ATIM_TIMX_COMP)
    {
        ...
        AFIO_REMAP_PARTIAL(AFIO_MAPR_TIM3_REMAP_PARTIALREMAP,AFIO_MAPR_TIM3_REMAP_FULLREMAP);  /*这里和TIM3没关系，复用PC6~PC9为CH1/CH2/CH3/CH4*/      
...
void atim_timx_cplm_pwm_init(uint16_t arr, uint16_t psc)
{
    ...
    sBreakDeadTimeConfig.LockLevel = TIM_LOCKLEVEL_OFF;                 /* 上电只能写一次，需要更新死区时间时只用能用此值 */
    sBreakDeadTimeConfig.BreakPolarity = TIM_BREAKPOLARITY_HIGH;          /*BKP = 1, BKIN高电平有效*/
